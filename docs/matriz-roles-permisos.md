...
| **Servicio** (Managed Identity de la Web App) | Storage Account — acción `generateUserDelegationKey` | Datos | Incluida en `Storage Blob Data Contributor` (sin asignación adicional) | Emitir el *user delegation key* con el que se firma el SAS temporal de lectura entregado al analista (endpoint `GET /documents/access-url`, sección 2.10) |
...
| **Analista de fraude** | Storage Account — contenedor `identity-documents` | Datos | _User Delegation SAS_ (sin rol RBAC permanente) | Acceder temporalmente a documentos de verificación de identidad para el análisis de un caso. Implementado en `GET /documents/access-url` (`api/app/storage/document_storage.py::generate_read_sas_url`): la Web App usa su propia Managed Identity para pedir un *user delegation key* a Azure AD y firmar con él un SAS de solo lectura acotado a un blob, válido 30 minutos — el analista nunca recibe un rol RBAC permanente ni una clave de cuenta |
